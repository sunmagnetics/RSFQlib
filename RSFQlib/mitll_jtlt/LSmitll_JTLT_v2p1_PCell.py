...
class PCELL(spira.PCell):
    ...
    def create_elements(self, elems):
        ...
        L3 = spira.RoutePath(port1=PJ2,port2=PN7,path=[(1.5*tp,3.18*tp),(1.6*tp,3.18*tp),(1.6*tp,2.98*tp),
                                                       (1.41*tp,2.98*tp),(1.41*tp,2.78*tp),(1.5*tp,2.78*tp)],width=L3_width,layer=M6)
        L4 = spira.RoutePath(port1=PN7,port2=PJ3,path=[(1.5*tp,2.3*tp),(1.41*tp,2.3*tp),(1.41*tp,2.1*tp),                                                       (1.6*tp,2.1*tp),(1.6*tp,1.9*tp),(1.5*tp,1.9*tp)],width=L4_width,layer=M6)
        L5_1 = spira.RoutePath(port1=PJ3,port2=PR1W,path=[(PJ3.x,PR1W.y)],width=L5_width,layer=M6)
        L5_2 = spira.RoutePath(port1=PR1E,port2=PQ,path=[(PQ.x,PR1E.y)],width=L5_width,layer=M6)
        ...
        LB2_1 = spira.RoutePath(port1=PN7,port2=PB2W,path=[(PN7.x,PB2W.y)],width=LB1_width,layer=M6)
...
class M4M5M6M7_connections(spira.Cell):
    ...
    def create_elements(self, elems):
        ...
        elems += spira.SRef(ls_conn_M4M5M6M7(),midpoint=(1.195*tp,2.335*tp),transformation=r90)
        elems += spira.SRef(ls_conn_M4M5M6M7(),midpoint=(2.665*tp,4.795*tp),transformation=m90)
        return elems
...
class junctions(spira.Cell):
    ...
    def create_elements(self, elems):
        elems += spira.SRef(ls_jj_162_sg(),midpoint=(1.5*tp,4.5*tp),transformation=r180,alias='J1')
        elems += spira.SRef(ls_jj_200_sg(),midpoint=(1.5*tp,3.5*tp),transformation=r90,alias='J2')
