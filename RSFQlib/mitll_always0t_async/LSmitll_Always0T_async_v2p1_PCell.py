...
class PCELL(spira.PCell):
    __name_prefix__ = "LSmitll_Always0T_async_v2p1"
    def create_elements(self, elems):
        ...
        LA = spira.Route90(port1=pptl_in,port2=pres_in_north,layer=M6,width=LA_width)
        LRA = spira.Route90(port1=pres_in_south,port2=pvia_in,layer=M6,width=LRA_width)
...
class junction_fill(spira.Cell):
    ...
    def create_elements(self, elems):
        ...
        elems += spira.SRef(ls_FakeJJ_3umx3um(),midpoint=(0.5*tp,4.2*tp))
        elems += spira.SRef(ls_FakeJJ_3umx3um(),midpoint=(0.5*tp,5*tp))
        elems += spira.SRef(ls_FakeJJ_3umx3um(),midpoint=(0.5*tp,5.835*tp))
        return elems
...
class M4M5M6M7_connections(spira.Cell):
    ...
    def create_elements(self, elems):
        ...
        elems += spira.SRef(ls_conn_M4M5M6M7(),midpoint=(0.68*tp,4.67*tp),transformation=T)
        elems += spira.SRef(ls_conn_M4M5M6M7(),midpoint=(0.68*tp,5.505*tp),transformation=T)
        elems += spira.SRef(ls_conn_M4M5M6M7(),midpoint=(0.68*tp,6.3625*tp),transformation=T)
        elems += spira.SRef(ls_conn_M4M5M6M7(),midpoint=(0.68*tp,6.75*tp),transformation=T)
        return elems
...
class resistors(spira.Cell):
    ...
    def create_elements(self, elems):
        ...
        elems += spira.SRef(ls_res_2(),midpoint=(0.5*tp,3.0375*tp),transformation=T)
        return elems
